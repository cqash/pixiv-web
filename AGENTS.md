...
src/api/       relay.ts（authorizedFetch/PostJson/GetJson + relayRequest 信封）、relayAuthApi.ts（register/refresh/ApiError）、
               ...
               sync.ts（/sync/v1：pushDomain 分批/pullDomain 续页/SyncFullRequiredError/stripTokenFields/游标持久化）、types.ts
...
8. **持久化键**：`arkpix.web.relay` / `arkpix.web.pixiv` / `arkpix.web.settings` / `arkpix.web.history` / `arkpix.web.sync`（每域游标）/ `arkpix.web.sync.meta`（写时间戳、墓碑、上次同步结果）。
