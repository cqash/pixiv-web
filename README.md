...
deploy.bat        # Windows：build 成功后清并拷贝 dist/* 到 ..\pixiv-relay\internal\web\dist\
```
...
- **生产**：`deploy.bat` 把 `dist/` 拷入 `pixiv-relay/internal/web/dist/`，后端 `go:embed` 托管，同源无 CORS。
...
协议契约权威文档：`ArkPix 客户端仓库 docs/backend-design.md`。
